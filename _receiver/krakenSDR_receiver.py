...
class ReceiverRTLSDR:
    def __init__(self, data_que, data_interface="eth", logging_level=10):
        ...
        self.port = 5000
        self.rec_ip_addr = (
            "127.0.0.1"  # Configured by the GUI prior to connection request
        )
        self.socket_inst = socket.socket()
        self.receiverBufferSize = (
            2**18
    ...
    def get_iq_online(self):
        ...
        if self.data_interface == "eth":
            self.socket_inst.sendall(
                str.encode("IQDownload")
            )  # Send iq request command
            ...
            if incoming_payload_size > 0:
                iq_samples_in = (
                    buffer[1024 : 1024 + incoming_payload_size].view(dtype=np.complex64)
    ...
    def receive_iq_frame(self):
        ...
        if incoming_payload_size > 0:
            ...
            iq_data_bytes = bytearray(
                total_bytes_to_receive + receiver_buffer_size
            )  # allocate array
            ...
            self.iq_samples = np.frombuffer(
                iq_data_bytes[0:total_bytes_to_receive], dtype=np.complex64
            ).reshape(self.iq_header.active_ant_chs, self.iq_header.cpi_length)
