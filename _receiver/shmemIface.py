...
class outShmemIface:
    def __init__(self, shmem_name, shmem_size, drop_mode=False):
        ...
        try:
            shmem_A = shared_memory.SharedMemory(
                name=shmem_name + "_A", create=False, size=shmem_size
            )
            shmem_A.close()
            # shmem_A.unkink()
        ...
        try:
            shmem_B = shared_memory.SharedMemory(
                name=shmem_name + "_B", create=False, size=shmem_size
            )
            shmem_B.close()
            # shmem_B.unkink()
        except FileNotFoundError as err:
        ...
        self.buffers.append(
            np.ndarray((shmem_size,), dtype=np.uint8, buffer=self.memories[0].buf)
        )
        self.buffers.append(
            np.ndarray((shmem_size,), dtype=np.uint8, buffer=self.memories[1].buf)
        )
        ...
        try:
            self.fw_ctr_fifo = os.open(
                "_data_control/" + "fw_" + shmem_name, os.O_WRONLY
            )
            self.bw_ctr_fifo = os.open(
                "_data_control/" + "bw_" + shmem_name, bw_fifo_flags
            )
    ...
    def wait_buff_free(self):
        if self.buffer_free[0]:
            ...
            try:
                ...
                self.logger.warning(
                    "Dropping frame.. Total: [{:d}] ".format(self.dropped_frame_cntr)
                )
            return 3
        return -1
