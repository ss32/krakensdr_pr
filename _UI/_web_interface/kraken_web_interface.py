...
daq_subsystem_path = os.path.join(
    os.path.join(os.path.dirname(root_path), "heimdall_daq_fw"), "Firmware"
)
daq_preconfigs_path = os.path.join(
    os.path.join(os.path.dirname(root_path), "heimdall_daq_fw"), "config_files"
)
...
class webInterface:
    def __init__(self):
        ...
        self.module_receiver.daq_rx_gain = [
            float(dsp_settings.get("gain_1", 1.4)),
            float(dsp_settings.get("gain_2", 1.4)),
        ]
        ...
        self.module_signal_processor.PR_clutter_cancellation = dsp_settings.get(
            "clutter_cancel_algo", "Wiener MRE"
        )
        self.module_signal_processor.max_bistatic_range = dsp_settings.get(
        ...
        self.max_bistatic_speed_kmh = (
            self.module_signal_processor.max_doppler
            * c
            / self.module_receiver.daq_center_freq
        ) * 3.6  # TODO: set this based on max_doppler in settings
    ...
    def save_configuration(self):
        ...
        data["data_interface"] = dsp_settings.get(
            "data_interface", "shmem"
        )  # settings.data_interface
        data["default_ip"] = dsp_settings.get(
            "default_ip", "0.0.0.0"
        )  # settings.default_ip
        ...
        data[
            "clutter_cancel_algo"
        ] = self.module_signal_processor.PR_clutter_cancellation
        ...
        data["logging_level"] = dsp_settings.get(
            "logging_level", 0
        )  # settings.logging_level
        data["disable_tooltips"] = dsp_settings.get(
            "disable_tooltips", 0
        )  # settings.disable_tooltips
...
def write_config_file_dict(param_dict):
    ...
    error_list = ini_checker.check_ini(
        ini_parameters, dsp_settings.get("en_hw_check", 0)
    )  # settings.en_hw_check)
    if len(error_list):
        for e in error_list:
...
if webInterface_inst.log_imagery or webInterface_inst.log_raw_radar:
    webInterface_inst.logger.info(
        f"Saving data to {webInterface_inst.data_output_directory}. Files will be owned by root."
    )
    webInterface_inst.logger.info(
        f"When finished run sudo -R $USER:$USER {webInterface_inst.data_output_directory}"
    )
...
for m in range(
    0, webInterface_inst.module_receiver.M + 1
):  # +1 for the auto decimation window selection
    fig_dummy.add_trace(
        go.Scatter(
            x=x,
            y=y,
            name="Channel {:d}".format(m),
            line=dict(color=trace_colors[m], width=2),
        )
    )
...
waterfall_init = [
    [-80] * webInterface_inst.module_signal_processor.spectrum_window_size
] * 50
...
app.layout = html.Div(
    [
        ...
        html.Div(
            [
                html.Img(
                    src="assets/kraken_interface_bw_pr.png",
                    style={
                        "display": "block",
                        "margin-left": "auto",
                        "margin-right": "auto",
...
def generate_config_page_layout(webInterface_inst):
    ...
    cfg_decimated_bw = (
        (daq_cfg_dict["sample_rate"]) / daq_cfg_dict["decimation_ratio"]
    ) / 10**3
    cfg_data_block_len = daq_cfg_dict["cpi_size"] / (cfg_decimated_bw)
    cfg_recal_interval = (
        daq_cfg_dict["cal_frame_interval"] * (cfg_data_block_len / 10**3)
    ) / 60
        ...
        html.Div(
            [
                ...
                dcc.Dropdown(
                    id="daq_rx_gain",
                    options=gain_list,
                    value=webInterface_inst.module_receiver.daq_rx_gain[0],
                    clearable=False,
                ...
                dcc.Dropdown(
                    id="daq_rx_gain_2",
                    options=gain_list,
                    value=webInterface_inst.module_receiver.daq_rx_gain[1],
                    clearable=False,
        ...
        html.Div(
            [
                ...
                dcc.Checklist(
                    ...
                    id="en_basic_daq_cfg",
                    className="field-body",
                    value=webInterface_inst.en_basic_daq_cfg,
                ),
            ],
            className="field",
        ),
        html.Div(
            [  # basic daq config id
                ...
                html.Div(
                    [
                        html.H2(
                            "DAQ Subsystem Reconfiguration", id="init_title_reconfig"
                        ),
    ...
    daq_status_card = html.Div(
        [
            ...
            html.Div(
                [
                    html.Div(
                        "Latency:", id="label_daq_dsp_latency", className="field-label"
                    ),
            ...
            html.Div(
                [
                    html.Div(
                        "IQ snyc:", id="label_daq_iq_sync", className="field-label"
                    ),
                    html.Div(
                        "LOSS",
                        id="body_daq_iq_sync",
            ...
            html.Div(
                [
                    html.Div(
                        "RF center frequecy [MHz]:",
                        id="label_daq_rf_center_freq",
                        className="field-label",
                    ),
                    html.Div(
                        "- MHz", id="body_daq_rf_center_freq", className="field-body"
                    ),
                ],
                className="field",
            ),
            html.Div(
                [
                    ...
                    html.Div(
                        "- MHz", id="body_daq_sampling_freq", className="field-body"
                    ),
                ],
                className="field",
            ),
    ...
    dsp_config_card = html.Div(
        [
            ...
            html.Div(
                [
                    html.Div(
                        ...
                        id="label_clutter_cancellation",
                        className="field-label",
                    ),
                    dcc.Dropdown(
                        id="clutter_cancel_algo",
                        options=[
                            {"label": "OFF", "value": "OFF"},
                            {"label": "Wiener MRE", "value": "Wiener MRE"},
                        ],
                        value=webInterface_inst.module_signal_processor.PR_clutter_cancellation,
                        style={"display": "inline-block"},
                        className="field-body",
                    ),
                ],
            ...
            html.Div(
                [
                    html.Div(
                        "Max Bistatic Range [km]:",
                        id="label_max_bistatic_range",
                        className="field-label",
                    ),
                    dcc.Dropdown(
                        id="max_bistatic_range",
                        options=[
                            {
                                "label": round(16 * bistatic_resolution_km, 1),
                                "value": 16,
                            },
                            {
                                "label": round(32 * bistatic_resolution_km, 1),
                                "value": 32,
                            },
                            {
                                "label": round(64 * bistatic_resolution_km, 1),
                                "value": 64,
                            },
                            {
                                "label": round(128 * bistatic_resolution_km, 1),
                                "value": 128,
                            },
                            {
                                "label": round(256 * bistatic_resolution_km, 1),
                                "value": 256,
                            },
                            {
                                "label": round(512 * bistatic_resolution_km, 1),
                                "value": 512,
                            },
                            {
                                "label": round(1024 * bistatic_resolution_km, 1),
                                "value": 1024,
                            },
                        ],
                        value=webInterface_inst.module_signal_processor.max_bistatic_range,
                        style={"display": "inline-block"},
                        className="field-body",
                    ),
                ],
            ...
            html.Div(
                [
                    html.Div(
                        "Max Bistatic Speed [km/h]:",
                        id="label_max_bistatic_speed_kmh",
                        style={"display": "inline-block"},
                        className="field-label",
                    ),
                    dcc.Input(
                        id="max_bistatic_speed_kmh",
                        value=abs(webInterface_inst.max_bistatic_speed_kmh),
                        type="number",
                        style={"display": "inline-block"},
                        debounce=True,
                        className="field-body-textbox",
            ...
            html.Div(
                [
                    html.Div(
                        "PR Persist", id="label_en_persist", className="field-label"
                    ),
                    dcc.Checklist(
                        options=option,
                        id="en_persist_check",
                        className="field-body",
                        value=en_persist_values,
                    ),
                ],
                className="field",
            ),
            html.Div(
                [
                    html.Div(
                        "Persist Decay:",
                        id="label_persist_decay",
                        style={"display": "inline-block"},
                        className="field-label",
                    ),
                    dcc.Input(
                        id="persist_decay",
                        value=webInterface_inst.pr_persist_decay,
                        type="number",
                        min=0,
                        max=1,
                        step=0.01,
            ...
            html.Div(
                [
                    html.Div(
                        "Dynamic Range (Min):",
                        id="label_dynrange_min",
                        style={"display": "inline-block"},
                        className="field-label",
                    ),
                    dcc.Input(
                        id="dynrange_min",
                        value=webInterface_inst.pr_dynamic_range_min,
                        type="number",
                        style={"display": "inline-block"},
                        debounce=True,
                        className="field-body-textbox",
            ...
            html.Div(
                [
                    html.Div(
                        "Dynamic Range (Max):",
                        id="label_dynrange_max",
                        style={"display": "inline-block"},
                        className="field-label",
                    ),
                    dcc.Input(
                        id="dynrange_max",
                        value=webInterface_inst.pr_dynamic_range_max,
                        type="number",
                        style={"display": "inline-block"},
                        debounce=True,
                        className="field-body-textbox",
    ...
    krakenpro_config_card = html.Div(
        [
            ...
            html.Div(
                [
                    html.Div(
                        "Kraken Pro API Key:",
                        id="label_kraken_pro_api_key",
                        style={"display": "inline-block"},
                        className="field-label",
                    ),
                    dcc.Input(
                        id="kraken_pro_api_key",
                        value=webInterface_inst.krakenpro_key,
                        type="text",
                        style={"display": "inline-block"},
                        debounce=True,
                        className="field-body-textbox",
...
def fetch_dsp_data():
        ...
        for data_entry in que_data_packet:
            if data_entry[0] == "iq_header":
                ...
                if webInterface_inst._update_rate_arr is None:
                    webInterface_inst._update_rate_arr = (
                        np.ones(webInterface_inst._avg_win_size)
                        * webInterface_inst.daq_update_rate
                    )
                webInterface_inst._update_rate_arr[
                    0 : webInterface_inst._avg_win_size - 2
                ] = webInterface_inst._update_rate_arr[
                    1 : webInterface_inst._avg_win_size - 1
                ]
                webInterface_inst._update_rate_arr[
                    webInterface_inst._avg_win_size - 1
                ] = webInterface_inst.daq_update_rate
                # webInterface_inst.page_update_rate = np.average(webInterface_inst._update_rate_arr)*0.8
            elif data_entry[0] == "latency":
                webInterface_inst.daq_dsp_latency = (
                    data_entry[1] + webInterface_inst.daq_cpi
                )
                ...
                webInterface_inst.logger.debug(
                    "Passive Radar RD Matrix data fetched from signal processing que"
                )
                doa_update_flag = 1
    ...
    if (
        webInterface_inst.pathname == "/config" or webInterface_inst.pathname == "/"
...
def update_daq_params(input_value, f0, gain, gain_2):
    ...
    webInterface_inst.config_daq_rf(
        f0, [gain, gain_2]
    )  # CARL: TO CHANGE THIS TO AUTO POPULATE EACH GAIN UP TO M RECEIVERS?
    return 1
...
def display_page(pathname):
    ...
    if pathname == "/":
        ...
        plot_pr()
        return [
            generate_pr_page_layout(webInterface_inst),
            "header_inactive",
            "header_inactive",
            "header_active",
        ]
...
@app.callback_shared(None, [Input("pr-graph", "clickData")])
...
def wr_pr_json(r_b):
    ...
    try:
        ...
        r = webInterface_inst.pool.apply_async(
            requests.post,
            kwds={"url": "http://127.0.0.1:8042/prpost", "json": jsonDict},
        )
...
def plot_pr():
    ...
    max_bistatic_distance_cells = (
        webInterface_inst.module_signal_processor.max_bistatic_range
    )
    bistatic_resolution = c / (
        webInterface_inst.module_receiver.iq_header.sampling_freq
    )
    bistatic_resolution_km = bistatic_resolution / 1000
    ...
    if (
        webInterface_inst.CAFMatrixPersist is None
        or webInterface_inst.CAFMatrixPersist.shape != CAFMatrix.shape
        or not webInterface_inst.en_persist
    ):
        webInterface_inst.CAFMatrixPersist = CAFMatrix
    else:
        webInterface_inst.CAFMatrixPersist = (
            np.maximum(webInterface_inst.CAFMatrixPersist, CAFMatrix)
            * webInterface_inst.pr_persist_decay
        )  # webInterface_inst.CAFMatrixPersist * 0.5 + CAFMatrix * 0.5
    ...
    CAFMatrixLog = resize(
        CAFMatrixLog, (maxImageY, maxImageX), order=1, anti_aliasing=True
    )
    ...
    if not webInterface_inst.pr_graph_reset_flag:
        ...
        pr_fig.update_layout(images=[go.layout.Image(source=img)])
        ...
        bistatic_speed_ms = (
            -webInterface_inst.module_signal_processor.max_doppler
            * c
            / webInterface_inst.module_receiver.daq_center_freq
        )
        bistatic_speed_kmh = bistatic_speed_ms * 3.6

        y_range = list(
            np.linspace(-bistatic_speed_kmh, bistatic_speed_kmh, y_height)
        )  # in Hz
        ...
        pr_fig.update_yaxes(
            title_text="Bistatic Speed [km/h]",
            color="rgba(255,255,255,1)",
            title_font_size=20,
            fixedrange=True,
            # tickfont_size=figure_font_size,
...
def plot_spectrum():
    ...
    if spectrum_fig == None:
        ...
        for m in range(np.size(webInterface_inst.spectrum, 0) - 1):
            spectrum_fig.add_trace(
                go.Scattergl(
                    ...
                    name="Channel {:d}".format(m),
                    line=dict(color=trace_colors[m], width=1),
                )
            )
        ...
        update_data = []
        for m in range(
            1, np.size(webInterface_inst.spectrum, 0)
        ):  # webInterface_inst.module_receiver.M+1):
            update_data.append(
                dict(
        ...
        app.push_mods(
            {
                "spectrum-graph": {
                    "extendData": [
                        update_data,
                        list(range(0, len(webInterface_inst.spectrum) - 1)),
                        len(webInterface_inst.spectrum[0, :]),
                    ]
                },
                "waterfall-graph": {
                    "extendData": [
                        dict(z=[[webInterface_inst.spectrum[1, :]]]),
                        [0],
                        50,
                    ]
                },
...
@app.callback(
    None, [Input(component_id="kraken_pro_api_key", component_property="value")]
)
...
def update_dsp_params(
    ...
    webInterface_inst.module_signal_processor.PR_clutter_cancellation = (
        clutter_cancel_algo
    )
...
def update_daq_ini_params(
    ...
    if ctx.triggered:
        ...
        if component_id == "daq_cfg_files":
            ...
            webInterface_inst.tmp_daq_ini_cfg = webInterface_inst.daq_ini_cfg_dict[
                "config_name"
            ]
            daq_cfg_dict = webInterface_inst.daq_ini_cfg_dict
            ...
            en_persist_values = [1] if webInterface_inst.en_persist else []
            en_pr_values = (
                [1] if webInterface_inst.module_signal_processor.en_PR else []
            )
        ...
        if (
            component_id == "cfg_data_block_len"
            or component_id == "cfg_decimated_bw"
            or component_id == "cfg_recal_interval"
        ):
            ...
            cfg_corr_size = (
                32768  # Reasonable value that never has problems calibrating
            )
            ...
            cfg_decimation_ratio = round(
                (cfg_sample_rate * 10**6) / (cfg_decimated_bw * 10**3)
            )

            cfg_cpi_size = round(
                (cfg_data_block_len / 10**3) * cfg_decimated_bw * 10**3
            )
    ...
    param_dict["require_track_lock_intervention"] = (
        1 if len(en_req_track_lock_intervention) else 0
    )
    ...
    if ctx.triggered:
        ...
        if (
            ...
            cfg_decimated_bw = (
                (int(cfg_sample_rate * 10**6)) / cfg_decimation_ratio
            ) / 10**3
            cfg_data_block_len = cfg_cpi_size / (cfg_decimated_bw)
            cfg_recal_interval = (
                cfg_cal_frame_interval * (cfg_data_block_len / 10**3)
            ) / 60
            ...
            app.push_mods(
                {
                    ...
                    "en_req_track_lock_intervention": {
                        "value": en_req_track_lock_intervention
                    },
...
@app.callback(
    Output("adv-cfg-container", "style"), [Input("en_advanced_daq_cfg", "value")]
)
...
@app.callback(
    Output("basic-cfg-container", "style"), [Input("en_basic_daq_cfg", "value")]
)
...
@app.callback(
    Output("system_control_container", "style"), [Input("en_system_control", "value")]
)
...
def reconfig_daq_chain(input_value, freq, gain):
    ...
    # Kill DAQ subsystem
    daq_stop_script = subprocess.Popen(
        ["bash", daq_stop_filename]
    ...
    webInterface_inst.module_receiver.daq_rx_gain = (
        daq_rx_gain  # settings.uniform_gain #daq_rx_gain
    )
...
class GracefulKiller:
    ...
    def exit_gracefully(self, signum, frame):
        webInterface_inst.logger.info(
            "Got kill signal. Cleaning up data permissions..."
        )
