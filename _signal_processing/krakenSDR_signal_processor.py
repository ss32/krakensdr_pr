    ...
    def run(self):
        ...
        while True:
            ...
            while self.run_processing:
                ...
                en_proc = (
                    self.module_receiver.iq_header.frame_type
                    == self.module_receiver.iq_header.FRAME_TYPE_DATA
                )  # or \
                ...
                if self.first_frame:
                    ...
                    self.spectrum = np.ones(
                        (self.channel_number + 1, self.spectrum_window_size),
                        dtype=np.float32,
                    )
                    self.first_frame = 0
                ...
                if en_proc:
                    ...
                    if self.en_spectrum and self.data_ready:
                        ...
                        )  # spectrum_signal #self.processed_signal #self.module_receiver.iq_samples #self.processed_signal
                        ...
                        for m in range(self.channel_number):
                            ...
                            f, Pxx_den = signal.welch(
                                ...
                                self.module_receiver.iq_header.sampling_freq
                                // first_decimation_factor,
                                nperseg=N_perseg,
                                nfft=N,
                                ...
                                window=(
                                    "tukey",
                                    0.25,
                                ),  # tukey window gives better time resolution for squelching #self.spectrum_window, #('tukey', 0.25), #self.spectrum_window,
                                # window=self.spectrum_window,
                                scaling="spectrum",
                            )

                            self.spectrum[1 + m, :] = np.fft.fftshift(
                                10 * np.log10(Pxx_den)
                            )
                    ...
                    if self.en_PR and self.data_ready and self.channel_number > 1:
                        ...
                        if self.PR_clutter_cancellation == "Wiener MRE":
                            surv_ch, w = Wiener_SMI_MRE(
                                ref_ch, surv_ch, td_filter_dimension
                            )
                        ...
                        )  # surv_ch * get_window(surv_ch.size) #det.windowing(surv_ch, "Hamming") #surv_ch * signal.tukey(surv_ch.size, alpha=0.25) #det.windowing(surv_ch, "hamming")
                ...
                que_data_packet.append(
                    [
                        "latency",
                        int(stop_time * 10**3)
                        - self.module_receiver.iq_header.time_stamp,
                    ]
                )
...
def Wiener_SMI_MRE(ref_ch: np.array, surv_ch: np.array, K: int) -> np.array:
    ...
    return (
        surv_ch - signal.oaconvolve(ref_ch, w)[0:N],
        w,
    )  # subtract the zero doppler clutter #oaconvolve saves us about 100-200 ms
...
def fast_w(R, r, K, R_mult):
    ...
    w = (
        lin.inv(R) @ r
    )  # np.dot(lin.inv(R), r)  # weight vector #matmul (@) may be slightly faster that np.dot for 1D, 2D arrays.
...
@jit(fastmath=True, nopython=False, cache=True)
def pruned_correlation(
    ref_ch: np.array, surv_ch: np.array, clen: int, N: int
) -> (np.array, np.array):
    """
    Description:
    -----------
    ...
    corr_batches = fft.fftshift(
        fft.ifft(corr_mult(xpw, bpw), axis=1, workers=4, overwrite_x=True)
    ).conj()[:, 0:clen]
    ...
    ypp = np.vstack(
        [yp[1:, :], np.zeros(cols, dtype=c_dtype)]
    )  # vstack appears to be faster than pad
...
def resize_and_align(no_sub_tasks, ref_ch, surv_ch, fs, fD_max, r_max):
    surv_ch_align = np.reshape(
        surv_ch, (no_sub_tasks, r_max)
    )  # shaping surveillance signal array into a matrix
    pad_zeros = np.expand_dims(np.zeros(r_max, dtype=c_dtype), axis=0)
    surv_ch_align = np.vstack(
        (surv_ch_align, pad_zeros)
    )  # padding one row of zeros into the surv matrix
    surv_ch_align = np.concatenate(
        (surv_ch_align[0:no_sub_tasks, :], surv_ch_align[1 : no_sub_tasks + 1, :]),
        axis=1,
    )

    ref_ch_align = np.reshape(
        ref_ch, (no_sub_tasks, r_max)
    )  # shaping reference signal array into a matrix
...
def cc_detector_ons(ref_ch, surv_ch, fs, fD_max, r_max):
    ...
    mx = np.zeros(
        (2 * Doppler_freqs_size + 1, r_max), dtype=c_dtype
    )  # memoize_zeros((2*Doppler_freqs_size+1, r_max), c_dtype) #np.zeros((2*Doppler_freqs_size+1, r_max),dtype = nb.c8)

    ref_ch_align, surv_ch_align = resize_and_align(
        no_sub_tasks, ref_ch, surv_ch, fs, fD_max, r_max
    )
    ...
    ref_fft = fft.fft(
        ref_ch_align, axis=1, overwrite_x=True, workers=4
    )  # pyfftw.interfaces.numpy_fft.fft(ref_ch_align_a, axis = 1, overwrite_input=True, threads=4) #fft.fft(ref_ch_align_a, axis = 1, overwrite_x=True, workers=4)
    surv_fft = fft.fft(
        surv_ch_align, axis=1, overwrite_x=True, workers=4
    )  # pyfftw.interfaces.numpy_fft.fft(surv_ch_align_a, axis = 1, overwrite_input=True, threads=4) #fft.fft(surv_ch_align_a, axis = 1, overwrite_x=True, workers=4)
    ...
    corr = fft.fft(
        corr_a, n=2 * no_sub_tasks, axis=0, workers=4, overwrite_x=True
    )  # Setting the output size with "n=.." is faster than doing a concat first.

    # crop and fft shift
    mx[0:Doppler_freqs_size, 0:r_max] = corr[
        2 * no_sub_tasks - Doppler_freqs_size : 2 * no_sub_tasks, 0:r_max
    ]
    mx[Doppler_freqs_size : 2 * Doppler_freqs_size + 1, 0:r_max] = corr[
        0 : Doppler_freqs_size + 1, 0:r_max
    ]
